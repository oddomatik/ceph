...
void ReplicatedPG::start_flush(OpContext *ctx, bool blocking)
{
  ...
  if (p != flush_ops.end()) {
    ...
    if (fop->ctx->op)
      osd->reply_op_error(fop->ctx->op, -EAGAIN);
...
void ReplicatedPG::finish_flush(hobject_t oid, tid_t tid, int r)
{
  ...
  if (r < 0) {
    reply_ctx(fop->ctx, -EAGAIN, obc->obs.oi.version,
